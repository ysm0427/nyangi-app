...
export default function App() {
  ...
  // 일정 데이터에 date 추가
  const [schedules, setSchedules] = useState([{ date: getTodayDateString(), time: "14:00", title: "동물병원 검진 🏥" }]);
  
  // 앨범 사진을 저장하는 상태 (실제 이미지 경로 저장)
  ...
  const handleAlbumUpload = (e) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        setAlbums({ ...albums, [currentCat]: [...albums[currentCat], event.target.result] });
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const deleteCareItem = (idx) => { const newItems = { ...careItems }; newItems[currentCat].splice(idx, 1); setCareItems(newItems); };
  const deleteExpense = (idx) => { const newExp = [...expenses]; newExp.splice(idx, 1); setExpenses(newExp); };
  const deleteSchedule = (idx) => { const newSch = [...schedules]; newSch.splice(idx, 1); setSchedules(newSch); };
        ...
        {careItems[currentCat].map((item, i) => {
          ...
          return (
            <div key={item.id} className="relative group">
              ...
              <button onClick={(e) => { e.stopPropagation(); deleteCareItem(i); }} className="absolute top-2 right-2 p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors"><Icons.Delete /></button>
            </div>
          );
        })}
      </div>
  ...
  const renderTab1 = () => (
    <div className="flex flex-col h-full bg-gray-50 p-4 space-y-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        <button onClick={() => { setModalState({ isOpen: true, type: 'expense' }); setFormData({ ...formData, title: '', amount: '' }); }} className="w-full py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold rounded-lg text-sm transition-colors">+ 지출 추가</button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        <button onClick={() => { setModalState({ isOpen: true, type: 'schedule' }); setFormData({ ...formData, title: '', date: getTodayDateString(), time: '12:00' }); }} className="w-full py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold rounded-lg text-sm transition-colors">+ 일정 추가</button>
      </div>
    </div>
  );
  ...
  const renderTab2 = () => (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {albums[currentCat].length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-400 space-y-3 opacity-60">
            <Icons.PhotoLibrary />
            <p className="text-sm font-bold">아직 추가된 사진이 없어요!</p>
          </div>
        ) : (
      ...
      <div className="p-4 flex gap-2 border-t border-gray-100 bg-white">
        ...
        <button 
          onClick={() => setIsAlbumEditMode(!isAlbumEditMode)}
          className={`px-5 py-3 font-bold rounded-xl shadow-sm transition-colors ${isAlbumEditMode ? 'bg-red-400 text-white hover:bg-red-500' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
        >
          {isAlbumEditMode ? '완료' : '수정'}
        </button>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col h-full bg-gray-50 p-6 overflow-y-auto">
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6">
        ...
        <ul className="space-y-3 text-sm text-gray-600">
          <li className="flex gap-2"><span>•</span><span>카메라 및 사진첩 직접 연동 기능 추가</span></li>
          <li className="flex gap-2"><span>•</span><span>일정 달력/시간 선택 기능 및 UI 개선</span></li>
          <li className="flex gap-2"><span>•</span><span>프로필 정보 수정 기능 업데이트</span></li>
        </ul>
      </div>
      <div className="text-center mt-auto py-4"><p className="text-[11px] font-bold text-gray-300">© 2026 NyangiApp Project.</p></div>
    </div>
  );
      ...
      <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl w-full max-w-[320px] shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="p-5 space-y-3">
            ...
            {modalState.type === 'careItem' && (
              <>
                ...
                <div className="flex gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide">
                  {["✨", "💧", "🥣", "💊", "🪮", "⚖️", "🧸", "🏥", "🐾", "🚿", "✂️"].map(emoji => (
                    <button key={emoji} onClick={() => setFormData({...formData, icon: emoji})} className={`text-2xl p-2 rounded-xl border shrink-0 transition-colors ${formData.icon === emoji ? 'bg-teal-50 border-teal-400 shadow-sm' : 'bg-gray-50 border-transparent hover:bg-gray-100'}`}>{emoji}</button>
                  ))}
                </div>
              </>
            )}
            ...
            {modalState.type !== 'editProfile' && (
              <input type="text" placeholder={modalState.type === 'careItem' ? "항목 이름 (예: 사료 급여)" : "내용을 입력하세요"} value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} autoFocus className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#A3E4D7] text-sm" />
            )}
            {modalState.type === 'careItem' && (
              <input type="text" placeholder="단위 (예: 회, ml, g, 분)" value={formData.unit} onChange={e => setFormData({...formData, unit: e.target.value})} className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#A3E4D7] text-sm" />
            )}
          ...
          <div className="px-5 py-3 bg-gray-50 flex justify-end gap-2 border-t border-gray-100">
            ...
            <button onClick={() => {
              ...
              else if (modalState.type === 'expense' && formData.title && formData.amount) setExpenses([...expenses, { detail: formData.title, amount: Number(formData.amount) }]);
              else if (modalState.type === 'schedule' && formData.title && formData.date && formData.time) setSchedules([...schedules, { date: formData.date, time: formData.time, title: formData.title }]);
              setModalState({ isOpen: false, type: null });
            }} className="px-4 py-2 text-sm font-bold bg-[#A3E4D7] text-gray-800 hover:bg-[#8fd9cb] rounded-lg transition-colors">
              {modalState.type === 'editProfile' ? '저장' : '추가'}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col">
          <div className="bg-white px-4 py-3 shrink-0 border-b border-gray-100 shadow-sm z-10">
            ...
            <div className="grid grid-cols-7 gap-1 text-center mb-1">
              {['일', '월', '화', '수', '목', '금', '토'].map((day, i) => (<div key={day} className={`text-[11px] font-bold py-1 ${i === 0 ? 'text-red-400' : i === 6 ? 'text-blue-400' : 'text-gray-400'}`}>{day}</div>))}
            </div>
            <div className="grid grid-cols-7 gap-1">
              {calendarDays.map((d, idx) => {
                ...
                return (
                  <button key={d} onClick={() => setTrackerDate(dateStr)} className={`flex flex-col items-center justify-start h-[46px] rounded-lg py-1 border transition-all ${isSelected ? 'border-teal-400 bg-teal-50 ring-1 ring-teal-400' : isToday ? 'border-gray-300 bg-gray-50' : 'border-transparent hover:bg-gray-50'}`}>
                    ...
                    {dayTotal > 0 && <span className="text-[9px] font-bold text-teal-600 bg-white/60 px-1 rounded-sm truncate w-full text-center leading-none">{Number.isInteger(dayTotal) ? dayTotal : dayTotal.toFixed(1)}</span>}
                  </button>
                );
              })}
            </div>
          ...
          <div className="p-4 flex flex-col items-center justify-center bg-gradient-to-b from-white to-teal-50/50 shrink-0 border-b border-teal-100">
             <div className="flex items-center gap-2 mb-2"><span className="text-xl">{tracker.icon}</span><span className="text-sm font-bold text-teal-700 bg-teal-100/50 px-3 py-1 rounded-full">{trackerDate} 총 기록</span></div>
             <p className="text-4xl font-black text-teal-600">{Number.isInteger(totalAmount) ? totalAmount : totalAmount.toFixed(1)}<span className="text-2xl text-teal-400 font-bold ml-1">{tracker.unit}</span></p>
          </div>
          <div className="p-4 bg-white shadow-sm shrink-0 border-b border-gray-100">
            <div className="flex gap-2 mb-3">
              {quickAmounts.map(amount => (<button key={amount} onClick={() => handleAddRecord(amount)} className="flex-1 py-2 bg-teal-50 text-teal-700 font-bold rounded-lg border border-teal-100 hover:bg-teal-100 transition-colors text-sm">+{amount}{tracker.unit}</button>))}
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={() => handleAddRecord(trackerInputAmount)} className="px-6 py-2.5 bg-[#A3E4D7] hover:bg-[#8fd9cb] text-gray-800 font-bold rounded-xl transition-colors shadow-sm">등록</button>
            </div>
          </div>
          <div className="flex-1 p-4 space-y-2 bg-gray-50">
            {dailyRecords.length === 0 ? (
              <div className="py-8 flex flex-col items-center justify-center text-gray-400 space-y-3"><div className="text-4xl opacity-50 grayscale">{tracker.icon}</div><p className="text-sm font-medium">아직 기록된 내용이 없어요!</p></div>
            ) : (
              dailyRecords.map((record, i) => (
                <div key={i} className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-100 animate-in fade-in slide-in-from-bottom-2 duration-200">
                  <div className="flex items-center gap-3"><div className="w-10 h-10 bg-teal-50 text-xl rounded-full flex items-center justify-center">{tracker.icon}</div><div><p className="font-bold text-gray-800 text-lg">{record.amount} <span className="text-sm font-medium text-gray-500">{tracker.unit}</span></p><p className="text-xs text-gray-400 font-medium">{record.time}</p></div></div>
                  <button onClick={() => handleDeleteRecord(i)} className="p-2 text-red-400 hover:bg-red-50 hover:text-red-500 rounded-full transition-colors"><Icons.Delete /></button>
                </div>
              ))
            )}
          </div>
        ...
        <div className="flex justify-around items-center bg-white border-t border-gray-200 pb-safe pt-2 px-2 shadow-[0_-10px_20px_rgba(0,0,0,0.05)] z-30 shrink-0 h-16">
          {[
            ...
            <button key={idx} onClick={() => setTabIdx(idx)} className={`flex flex-col items-center justify-center w-full h-full transition-colors ${tabIdx === idx ? 'text-[#A3E4D7]' : 'text-gray-400 hover:text-gray-600'}`}>
              <div className="mb-1">{item.icon}</div><span className="text-[10px] font-bold">{item.label}</span>
            </button>
          ))}
        </div>
        {renderModal()}
